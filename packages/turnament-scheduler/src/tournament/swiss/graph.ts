...
import type { GraphEdge, PlayerWithResults } from "../../types";
 ...
 * @param players
 */
export const calcHighestScore = (players: PlayerWithResults[]): number =>
...
const makeNodes = (length: number) =>
	Array.from<number, number>({ length }, (v, k) => k + 1);
/**
 *  Returns graph edges
 * @param arr Nodes
 */
 ...
 * @param players
 */
export const makeWeightedGraph = (
	players: PlayerWithResults[],
): GraphEdge[] => {
	const highestScore = calcHighestScore(players);
	const unweightedGraph = makeUnweightedGraph(players);

	// we use indexes instead of IDs because MWM algorithm requires that in pairPlayer function
...
export function calcEdgeWeight(
	...
	const numPlayedOpponents = countOccurrences(player1.opponents);
