...
/**
 * Folds array into pairs of neighboring elements. Ignores last element if array has odd length.
 */
export const toPairs = (arr: number[]): Pairing[] =>
	arr.reduce<Pairing[]>(
		(acc, _, index, array) =>
			index % 2 === 0 && index + 1 < array.length
				? acc.concat([[array[index], array[index + 1]]])
				: acc,
		[],
	);
...
export const pairPlayers =
	...
	(players: PlayerWithResults[]): Pairing[] => {
		...
		// filter out players with no loses for upper bracket, map to IDs only
