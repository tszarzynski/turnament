...
import type { Pairing, PlayerWithResults, PlayerWithStats } from "../types";
...
function playMatch([pr1, pr2]: Pairing, players: PlayerWithResults[]) {
	...
	if (!pl1 || !pl2) throw new Error("Player ID cannot be found");
...
function playBye([pr1, pr2]: Pairing, players: PlayerWithStats[]) {
	...
	if (!pl) throw new Error("Player ID cannot be found");
